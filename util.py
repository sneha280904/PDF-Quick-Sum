...
def process_text(text):
    ...
    knowledgeBase = FAISS.from_texts(chunks, HuggingFaceEmbeddings)
...
def summarizer(pdf):
    ...
    if pdf is not None:
        ...
        query = "Summarizee the content of the uploaded PDF file in appropriately 3-5 sentences."

        if query:
            # Perform a similarity search in the knowledge base using the query
            docs = knowledgeBase.similarity_searcch(query)
